...
module warp_decoder_tb;
  ...
  initial begin
    expect_valid({`OP_LDI, 3'd6, 8'hA5}, `OP_LDI, 3'd6, 3'd0, 3'd0, 16'h00A5);
    expect_valid({`OP_ADD, 3'd1, 3'd2, 3'd3, 2'd0}, `OP_ADD, 3'd1, 3'd2, 3'd3, 16'd0);
    expect_valid({`OP_TID, 3'd4, 8'd0}, `OP_TID, 3'd4, 3'd0, 3'd0, 16'd0);
    expect_valid({`OP_HALT, 11'd0}, `OP_HALT, 3'd0, 3'd0, 3'd0, 16'd0);

    expect_invalid({`OP_VADD, 3'd1, 3'd2, 3'd3, 2'd0});
    expect_invalid({`OP_LOAD, 3'd1, 3'd2, 5'd0});
