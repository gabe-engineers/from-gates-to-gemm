...
package control_helpers_pkg;
  ...
  function automatic logic is_immediate_writeback_op(input logic [4:0] opcode);
    begin
      is_immediate_writeback_op = opcode == `OP_LOAD || opcode == `OP_LDI ||
          opcode == `OP_LUI;
    end
  endfunction
  ...
  function automatic [1:0] writeback_source_for_opcode(input logic [4:0] opcode);
    begin
      if (opcode == `OP_TID)
        writeback_source_for_opcode = WB_THREAD_ID;
      else if (is_immediate_writeback_op(opcode))
        writeback_source_for_opcode = WB_IMMEDIATE;
      else
        writeback_source_for_opcode = WB_ALU;
    end
  endfunction
