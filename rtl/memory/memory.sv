...
module memory (
    ...
    input  [15:0] gpu_address     [7:0],
    input  [ 7:0] gpu_write_enable,
    input  [15:0] gpu_write_data  [7:0],
    output [15:0] cpu_read_data,
    output [15:0] gpu_read_data   [7:0]
);
  ...
  generate
    for (genvar i = 0; i < 512; i++) begin : memory_words
      ...
      for (genvar gpu_i = 0; gpu_i < 8; gpu_i++) begin : gpu_writes
        assign gpu_writes_this_word[gpu_i] = gpu_write_enable[gpu_i] && (gpu_address[gpu_i] == i);
      end
      ...
      assign gpu_write_data_this_word =
          gpu_writes_this_word[7] ? gpu_write_data[7] :
          gpu_writes_this_word[6] ? gpu_write_data[6] :
          gpu_writes_this_word[5] ? gpu_write_data[5] :
          gpu_writes_this_word[4] ? gpu_write_data[4] :
          gpu_writes_this_word[3] ? gpu_write_data[3] :
          gpu_writes_this_word[2] ? gpu_write_data[2] :
          gpu_writes_this_word[1] ? gpu_write_data[1] :
                                   gpu_write_data[0];
    ...
    for (genvar gpu_i = 0; gpu_i < 8; gpu_i++) begin : gpu_reads
      assign gpu_read_data[gpu_i] = register_out[gpu_address[gpu_i]];
    end
  endgenerate
